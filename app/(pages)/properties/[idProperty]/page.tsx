...
export default async function Page({ params }: { params: { idProperty: string } }) {
  ...
  const property: Property = await getPropertyById(p.idProperty);
  if (!property) {
    notFound();
  }
  log(property);
  ...
  return (
    <main className="flex-1 max-w-350 mx-auto flex flex-col mt-20 gap-6 mb-10 px-5">
      <header>
      ...
      <section className="flex gap-4 items-start">
        <div className="grid grid-cols-[370px_220px_220px] grid-rows-2 gap-2 h-89">
          ...
          <div className="col-span-2 row-span-2 h-full grid grid-cols-2 grid-rows-2 gap-2">
        ...
        <article className="bg-white border border-[#F5F5F5] p-6 rounded-xl flex flex-col gap-6 w-105">
